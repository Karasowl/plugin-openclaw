# Claude Agent Coordination Guide

This file defines the required coordination protocol for Claude Code inside this repository.

## Identity

- Agent ID: `claude-agent`
- Peer agent: `codex-agent`
...
- If a file is locked by `codex-agent`, do not edit it.
- Before editing any file, register your own lock in `.agent-locks.json`.
- When you finish, remove only your own locks.
- Never delete or overwrite a lock owned by `codex-agent`.
- If you need a file that `codex-agent` has locked, write a message to `.agent-mail.json`.
...
5. Verify that none of those files are locked by `codex-agent`.
...
3. If the path is already locked by `codex-agent`, do not touch it.
...
- You need a file locked by `codex-agent`.
...
- If a file is urgent and locked by `codex-agent`, send mail and work elsewhere.
...
Never commit while leaving stale locks owned by `claude-agent`.

## Quick operational example

1. Read `.agent-config.json`, `.agent-locks.json`, `.agent-mail.json`.
2. See that `src/plugin.ts` is free.
3. Add a lock for `src/plugin.ts` with `agent: "claude-agent"`.
