  ...
  return (event: MessageSendingEvent, ctx: MessageContext): MessageSendingResult | void => {
    ...
    if (isSessionDenied(sessionKey)) {
      ...
      const vars: Record<string, string> = {
        ...
        cost: String(sessionAccess?.costPerMessage ?? config.groups.costPerMessage),
        cooldown: String(sessionAccess?.cooldownSeconds ?? config.groups.cooldownSeconds),
      };
      ...
      } else if (reason === "custom_response" && sessionAccess?.customResponse) {
        message = sessionAccess.customResponse;
      } else {
        message = messaging?.templates?.insufficient_credits
          ? interpolateTemplate(messaging.templates.insufficient_credits, vars)
          : `⛔ You don't have enough credits. Your balance: ${balance}. You need ${sessionAccess?.costPerMessage ?? config.groups.costPerMessage} credit(s) to interact.`;
      }
