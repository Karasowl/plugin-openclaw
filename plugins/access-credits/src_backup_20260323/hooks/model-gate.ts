...
import { getSessionAccessInfo, isSessionDenied, isSessionTriggered } from "../gate-state.js";
...
export function createModelGateHandler(
  ...
  return (event: BeforeModelResolveEvent, ctx: AgentContext): BeforeModelResolveResult | void => {
    ...
    if (!isSessionTriggered(sessionKey)) {
      return undefined;
    }
    ...
    if (!isSessionDenied(sessionKey)
      && sessionAccess.contextKind === "direct"
      && config.directMessages.allowModelChoice
      && sessionAccess.selectedModelId) {
      return { modelOverride: sessionAccess.selectedModelId };
    }
