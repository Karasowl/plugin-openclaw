...
export interface CreditsStore {
  ...
  setUserPreferences(userId: string, preferences: Partial<UserPreferences>): UserAccount;
...
function normalizeUserAccount(user: UserAccount): UserAccount {
  return {
    ...user,
...
export function createCreditsStore(
  ...
  function getData(): CreditsStoreData {
    ...
    const data = raw ?? { users: {}, transactions: [] };
    let dirty = false;
  ...
  return {
    getUser(userId: string): UserAccount | null {
      ...
      return user ? normalizeUserAccount(user) : null;
    },

    getOrCreateUser(userId: string, displayName?: string): UserAccount {
      const data = getData();
      if (data.users[userId]) {
        return normalizeUserAccount(data.users[userId]);
      }
    ...
    getAllUsers(): UserAccount[] {
      const data = getData();
      return Object.values(data.users).map((user) => normalizeUserAccount(user));
    },
