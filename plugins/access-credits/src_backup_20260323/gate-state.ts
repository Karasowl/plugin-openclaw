...
import type { AccessContextKind } from "./config.js";

const SESSION_TTL_MS = 5 * 60 * 1000; // 5 minutes

export type DenialReason = "no_credits" | "cooldown" | "custom_response";
...
const sessionAccess = new Map<string, SessionAccessInfo>();
...
export function markSessionTriggered(sessionKey: string): void {
  ...
  sessionAccess.delete(sessionKey);
...
export function setSessionAccessInfo(sessionKey: string, info: SessionAccessInfo): void {
  sessionAccess.set(sessionKey, info);
}

export function getSessionAccessInfo(sessionKey: string): SessionAccessInfo | null {
  pruneExpired();
  return sessionAccess.get(sessionKey) ?? null;
}
